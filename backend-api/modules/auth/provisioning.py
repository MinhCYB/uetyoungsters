...
class StudentCreate(InviteInput):
...
def create_student(class_id: str, payload: StudentCreate, db: Session = Depends(get_db), actor: User = Depends(require_roles(Role.HOMEROOM_TEACHER))):
    ...
    if db.scalar(select(User).where(User.email == email)):
        raise HTTPException(409, "Email đã có tài khoản")
    # A direct demo account supersedes any old, unaccepted invitation for the
    # same address; no new invitation token is generated.
    db.execute(delete(Invitation).where(
        Invitation.email == email,
        Invitation.role == Role.STUDENT,
    ...
    profile_data = payload.model_dump(
        exclude={"email", "display_name", "password"}, exclude_none=True
    )
    ...
    db.flush()
    audit(
        db, actor, "STUDENT_ACCOUNT_CREATED", "USER", user.id,
        details={"class_id": class_id, "profile_type": profile.profile_type, "email": email},
    )
    db.commit()
    return {
        "id": user.id,
