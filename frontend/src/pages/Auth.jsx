...
export function ProfessionalRegister({ onAuthenticated }) {
  const [form,setForm]=useState({display_name:'',email:'',password:'',graduated:false}); const [error,setError]=useState('');
  ...
  return <AuthShell eyebrow="Tài khoản cá nhân" title="Đăng ký cho người đã ra trường" description="Chỉ dành cho người đã tốt nghiệp và không còn thuộc tài khoản do nhà trường quản lý."><form className="auth-form" onSubmit={submit}><Field label="Tên hiển thị" required value={form.display_name} onChange={e=>setForm({...form,display_name:e.target.value})}/><Field label="Email" type="email" required value={form.email} onChange={e=>setForm({...form,email:e.target.value})}/><Field label="Mật khẩu (tối thiểu 8 ký tự)" type="password" minLength="8" required value={form.password} onChange={e=>setForm({...form,password:e.target.value})}/><label className="auth-confirm"><input type="checkbox" required checked={form.graduated} onChange={e=>setForm({...form,graduated:e.target.checked})}/><span>Tôi xác nhận mình đã ra trường</span></label>{error&&<div className="auth-error">{error}</div>}<button className="primary auth-submit">Tạo tài khoản →</button></form><div className="auth-links"><button onClick={()=>go('/login')}>Đã có tài khoản? Đăng nhập</button></div></AuthShell>;
}
